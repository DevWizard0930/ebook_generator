...
class BookBuilder:
    ...
    def log_to_airtable(self, book_data: Dict, concept: Dict, outline: Dict, 
                       word_count: int, generation_time: float, cover_path: str = None,
                       book_files: Dict = None, upload_info: Dict = None, 
                       publishing_info: Dict = None) -> Optional[str]:
        """Log all information to Airtable"""
        try:
            # Log book creation
            record_id = self.airtable_logger.log_book_creation(
                book_data=book_data,
                concept=concept,
                outline=outline,
            ...
            # Log cover generation if available
            if cover_path:
                self.airtable_logger.log_cover_generation(record_id, cover_path, 0)
            
            # Log file creation if available
            if book_files:
                self.airtable_logger.log_file_creation(record_id, book_files, 0)
            
            # Log Google Drive upload if available
            if upload_info:
                self.airtable_logger.log_google_drive_upload(record_id, upload_info, 0)
            
            # Log StreetLib publishing if available
            if publishing_info:
                self.airtable_logger.log_streetlib_publishing(record_id, publishing_info, 0)
    ...
    def build_book(self, genre: Optional[str] = None, title: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            upload_info = self.upload_to_google_drive(
                book_title=outline['title'],
                book_files=book_files,
                cover_path=cover_path
            )
            
            # Step 8: Generate StreetLib metadata
            book_data = {
                'title': outline['title'],
            ...
            streetlib_metadata = self.generate_streetlib_metadata(book_data)
            
            # Step 9: Publish to StreetLib
            publishing_info = self.publish_to_streetlib(
                metadata=streetlib_metadata,
                cover_path=cover_path,
                book_files=book_files
            )
            ...
            record_id = self.log_to_airtable(
                book_data=book_data,
                concept=concept,
                outline=outline,
                word_count=concept['word_count'],
            ...
            logger.info(f"Google Drive: {'Success' if upload_info else 'Failed'}")
            logger.info(f"StreetLib: {'Success' if publishing_info.get('success') else 'Failed'}")
            ...
            try:
                self.airtable_logger.log_error("", str(e), "Book Building")
            except:
                pass
